...
describe MyCoffeeCompiler do
  let(:compiler) { MyCoffeeCompiler.new }
  context "comments" do
    it("preserves comments") do
      compiler.compile("# anything after a hash").should == "// anything after a hash"
    end
  ...
  context "string" do
    it "simple" do
      compiler.compile('"simple"').should == '"simple"'
    end
    it "escaped string" do
      compiler.compile('"a \\" b"').should == '"a \\" b"'
    end
  end
  ...
  context "number" do
    it "42" do
      compiler.compile('42').should == '42' 
    end 
    it "4.2" do
      compiler.compile('4.2').should == '4.2' 
    end 
    it "-4.2E24" do
      compiler.compile('-4.2E24').should == '-4.2E24' 
    end 
  end
